...
buffer = {
  ...
  loadRam = function(self, num)
    if type(num) == 'number' then
      if num <= #self.ram then
        local temp = {}
        for i=1,num do
...
main = {
  ...
  updateMsg = function(self)
    if self.selection < 2 and self.showingMessage >= 1 then
      draw:message(buffer:loadRam(self.showingMessage)[self.showingMessage])
    elseif self.selection > 2 and self.showingMessage <= #buffer:loadRam() then
      draw:message(buffer:loadRam(self.showingMessage)[self.showingMessage])
    else
    end
  end;
...
buffer:write({"HELLOW WORLD", "YOU ARE", "A BUNCH OF", "CUNTS!"})
...
draw:message(buffer:loadRam(1)[1]) -----------
