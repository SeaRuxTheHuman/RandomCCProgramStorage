----game engine project
    -->TO:DO<--
--add ability to use interactables without mouse.

--re:write code to be more effeciant.
  -- >timer based screen updates.
  -- >less use of term.clear().
  -- >re-work table data saveing/loading system to concat all table data into one "Mega Table".
  -- >add tArgs so the engine can easily run game data from an external table of tables.
  -- >re-work how actions are handled
  -- >re-work how key/mouse input is handled
  -- >add menu & inventory systems that save player either data to a dedicated part of the "Mega Table" or to a seperate file
  -- >look into makeing a permantly in-engine table of functions that basically clone  what computercraft api's do,
  --  this is so porting to other platforms (such as OC) only needs to be done in this "Basic Functions" table, and not a full code re-write
  -- >add  transparnt/passthroguh tiles.
  -- >add ability for transparent tiles to have functions when walked on?
  -- >add map scrolling and mulipul screen size support
  -- >add ability for external "Mega Table" deifned menus
  -- >add multi map support
  -- >add tereport chars for building interiors
  
--add method of pulling game table/s from modems, http api, floppy disks, ect.
--look into how game files can be organized efficiantly for floppy disk distribution.
--keep all external gameTable/s in one file

-- FAR FUTURE --> make dedicated map editor that can define everthing visible on screen with ease so
              --  codeing dose not require typeing in every pixel on screen, instead only what they do and other
              --  "more important" information. (keep editor non-public as not to create too many skript kiddiez and
              --  give my code/program a negaitive stigma for being used to make trash games.)
...
obj = {
  ...
  player = { --player info, movement, interactions, stuff here
	...
	actKey = function(self,lTableA,lTableB)
    --find an interactable next to the player, if one is found
    --then use said function with obj:run()
  end;
...
input = {
  ...
  key = function(self,key,lTableA,lTableB)
	...
	elseif key == keys.enter then
	  obj.player:actKey(lTableA,lTableB)
